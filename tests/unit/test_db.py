...
def test_sslmode_disable_sets_ssl_false():
    url, connect_args = _prepare_async_url("postgresql://user:pass@localhost/db?sslmode=disable")
    assert connect_args.get("ssl") is False
    assert "sslmode" not in url


def test_other_sslmode_is_stripped_without_ssl_arg():
    url, connect_args = _prepare_async_url("postgresql://user:pass@localhost/db?sslmode=require")
...
def test_already_asyncpg_url_is_unchanged():
    url, connect_args = _prepare_async_url("postgresql+asyncpg://user:pass@localhost/db")
